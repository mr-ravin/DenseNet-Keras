...
def binary_search_connections(start,end,list_keys):  ### 384- 20/12     512- 22/14   640- 23.9/15 
  if end-start>0:
    ...
    binary_search_connections(start+1,mid-1,list_keys)
    binary_search_connections(mid+1,end-1,list_keys)
...
def dense_block(x, stage, nb_layers, nb_filter, growth_rate, dropout_rate=None, weight_decay=1e-4, grow_nb_filters=True):
    ...
    concat_feat = x
    list_x=[]
    list_keys=gen_list(nb_layers)
    binary_search_connections(0,nb_layers-1,list_keys)
    for i in range(nb_layers):
        branch = i+1
        x = conv_block(concat_feat, stage, branch, growth_rate, dropout_rate, weight_decay)
        concat_feat = merge([concat_feat, x], mode='concat', concat_axis=concat_axis, name='concat_'+str(stage)+'_'+str(branch))
        list_x.append(concat_feat)
        if list_keys[i]!=[]:
          concat_feat = merge([concat_feat, list_x[list_keys[i][0]]], mode='concat', concat_axis=concat_axis, name='concat_fin'+str(stage)+'_'+str(branch))
